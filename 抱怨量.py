...
### LastEditTime: 2019-11-18 17:26:25
### Description: 计算抱怨量
'''
...
def get_bao_list(excel_file, excel_sheet, column_judge, column_name):
    '''
    ...
    print(f'正在处理:{excel_file}')
    # 下载下来的excel像是gbk格式
    df = pd.DataFrame(pd.read_excel(excel_file, sheet_name=excel_sheet))[[column_judge, column_name]]
    # 以处理人工号计算频次
...
if __name__ == "__main__":
    ...
    dict_baoyuan = get_bao_list(file_baoyuan, '清单', '', '受理人工号')
    file_lvtong = f'{os.path.dirname(__file__)}\绿通单清单2019年10月.xlsx'
    dict_lvtong = get_bao_list(file_lvtong, '1', '', '处理人工号')
    n = 0
    for i in dict_name:
